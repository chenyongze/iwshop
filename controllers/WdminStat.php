...
class WdminStat extends Controller {
    ...
    public function check_wechat_stat_status() {
        if (APPID == "__APPID__" || APPID == "" || APPSECRET == '__APPSECRET__' || APPSECRET == "") {
            ...
            // 获取总数
            $tcount = $this->Dao->select('')->count()->from(TABLE_USER_CUMULATE)->getOne(false);
            // 最新日期
            $datelast = date('Y-m-d', strtotime("-1 day"));
            // 接口最终日期
            if ($tcount > 0) {
                // 获取统计最新日期
                $datendStr = $this->Dao->select('ref_date')->from(TABLE_USER_CUMULATE)->orderby('ref_date')->desc()->getOne(false);
    ...
    public function getUserStat($Q) {
        ...
        $control = $Q->control;
        #$QueryMonth = isset($_GET['date']) ? $_GET['date'] : date("Y-m");
        $data = $this->Dao->select()->from(TABLE_USER_SUMMARY)->orderby('ref_date')->desc()->limit($limit)->exec();
        ...
        if ($control == 'cumulate_user') {
            $data = $this->Dao->select()->from(TABLE_USER_CUMULATE)->orderby('ref_date')->desc()->limit($limit)->exec();
            $data = array_reverse($data);
            $r = array();
            foreach ($data as $s) {
                $r['x'][] = $s['ref_date'];
