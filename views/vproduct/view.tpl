                    ...
                    <div>
                        {if $productInfo.product_prom eq 1}
                            {foreach from=$specs item=sp}
                                <input type='hidden' class='spec-hashs' data-stock="{$sp.instock}" value='{$sp.spec_det_id1}-{$sp.spec_det_id2}' data-price='{($sp.sale_price * ($productInfo.product_prom_discount / 100))|string_format:"%.2f"}' data-market-price="{$sp.market_price}" data-id="{$sp.id}" />
                            {/foreach}
                        {else}
                            {foreach from=$specs item=sp}
                                <input type='hidden' class='spec-hashs' data-stock="{$sp.instock}" value='{$sp.spec_det_id1}-{$sp.spec_det_id2}' data-price='{($sp.sale_price * $discount)|string_format:"%.2f"}' data-market-price="{$sp.market_price}" data-id="{$sp.id}" />
                            {/foreach}
                        {/if}
                    </div>
