{strip}{section name=i loop=$product_list}
        <section class="cartListWrap clearfix" id="cartsec{$product_list[i].product_id}">
            ...
            <img alt="{$product_list[i].product_name}" width="100" height="100" src="{if $config.usecdn}{$config.imagesPrefix}product_hpic/{$product_list[i].catimg}_x120{else}static/Thumbnail/?w=200&h=200&p={$config.productPicLink}{$product_list[i].catimg}{/if}" />
