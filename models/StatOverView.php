...
class StatOverView extends Model {
    ...
    public function getWechatSummary() {
        ...
        // 获取总数
        $tcount = $this->Dao->select('')->count()->from(TABLE_USER_SUMMARY)->getOne();
        ...
        if ($tcount > 0) {
            // 获取统计最新日期
            $datendStr = $this->Dao->select('ref_date')->from(TABLE_USER_SUMMARY)->orderby('ref_date')->desc()->getOne();
        ...
        if (sizeof($UserSummary['list']) > 0) {
            foreach ($UserSummary['list'] as $list) {
                if (!$this->Dao->insert(TABLE_USER_SUMMARY, 'ref_date, user_source, new_user, cancel_user')
                                ->values($list)->exec()) {
                    continue;
                } else {
                    $count++;
        ...
        if (sizeof($UserCumulate['list']) > 0) {
            foreach ($UserCumulate['list'] as $list) {
                if (!$this->Dao->insert(TABLE_USER_CUMULATE, 'ref_date, user_source, cumulate_user')
                                ->values($list)->exec()) {
                    continue;
                } else {
                    $count++;
