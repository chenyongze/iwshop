...
class CreditExchange extends Model {
    ...
    public function getReq($id) {
        if ($id > 0) {
            return $this->Dao->select('product_credits')->from(TABLE_CREDIT_EXCHANGE)->where("product_id = $id")->getOne();
        }
        return false;
    }
    ...
    public function getList($cache = true) {
        return $this->Dao->select("ex.*, pd.product_name, pd.catimg")->from(TABLE_CREDIT_EXCHANGE)->alias('ex')
                        ->leftJoin(TABLE_PRODUCTS)->alias('pd')
                        ->on("pd.product_id = ex.product_id")->exec($cache);
    }
