...
class Product extends Model {
    ...
    public function getDeletedProducts($limit = 100, $cache = true) {
        $SQL = "SELECT po.*,ps.sale_prices,psl.serial_name FROM `products_info` po "
                ...
                . "LEFT JOIN `product_serials` psl ON psl.id = po.product_serial WHERE `is_delete` = 1 LIMIT $limit;";
    ...
    public function getAllCats($catParent = 0, $cache = false) {
        ...
        foreach ($Lst as &$l) {
            ...
            // 商品数量
            $l['pdcount'] = $this->Dao->select('')->count('*')->from(TABLE_PRODUCTS)->where("product_cat = $l[cat_id]")->aw('`is_delete` <> 1')->getOne();
            $l['open'] = 'true';
     ...
     * @return string
     */
    public function getURI($id, $comid = false) {
        global $config;
        if (is_numeric($id)) {
            if (is_numeric($comid)) {
                $productURI = $domain . "?/vProduct/view/id=$id&com=$comid&showwxpaytitle=1";
            } else {
                $productURI = $domain . "?/vProduct/view/id=$id&showwxpaytitle=1";
            }
            return $productURI;
        }
    }
    ...
    public function upReadi($pid) {
        ...
        // readi
        return $this->Dao->update(TABLE_PRODUCTS)->set(array('product_readi' => 'product_readi + 1'), true)->where("`product_id` = $pid")->exec();
    }
